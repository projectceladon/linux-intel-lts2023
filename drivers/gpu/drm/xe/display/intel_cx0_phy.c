...
static int intel_c20_compute_hdmi_tmds_pll( struct intel_encoder *encoder,  u64 pixel_clock, struct intel_c20pll_state *pll_state)
{
	u64 datarate;
	u64 mpll_tx_clk_div;
	u64 vco_freq_shift;
				   ...
				   struct intel_encoder *encoder)
{
	...
	if (intel_crtc_has_type(crtc_state, INTEL_OUTPUT_HDMI)) {
		if (intel_c20_compute_hdmi_tmds_pll(encoder, crtc_state->port_clock,
...
static u8 cx0_power_control_disable_val(struct intel_encoder *encoder, enum phy phy)
{
	struct drm_i915_private *i915 = to_i915(encoder->base.dev);
...
static void intel_cx0pll_disable(struct intel_encoder *encoder)
{
	...
	intel_cx0_powerdown_change_sequence(i915, encoder->port, INTEL_CX0_BOTH_LANES,
					    cx0_power_control_disable_val(encoder, phy));
