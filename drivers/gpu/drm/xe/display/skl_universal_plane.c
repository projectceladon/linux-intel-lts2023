			       ...
			       int ccs_plane)
{
	...
	u32 alignment = plane->min_alignment(plane, fb, ccs_plane);
	int hsub;
	int vsub;
				 ...
				 int *x, int *y, u32 *offset)
{
	...
	const u32 alignment =  plane->min_alignment(plane, fb, 0);
...
static int skl_check_nv12_aux_surface(struct intel_plane_state *plane_state)
{
	...
	if (ccs_plane) {
		u32 aux_offset = plane_state->view.color_plane[ccs_plane].offset;
		u32 alignment =  plane->min_alignment(plane, fb, uv_plane);
